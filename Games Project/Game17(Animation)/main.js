...
const toggleAudio = () => {
  AUDIO.WARZONE.muted = AUDIO.CLACK_ONE.muted = AUDIO.CLACK_TWO.muted = !AUDIO.WARZONE.muted;
  if (!AUDIO.WARZONE.muted) AUDIO.WARZONE.play();else AUDIO.WARZONE.pause();
};

document.querySelector("#volume").addEventListener("input", toggleAudio);
set("#original-code-block", {
  x: 25,
  y: 15,
  scale: 0
});
...
const TL = timeline({
  ...
  yPercent: 3,
  ...shared
}).to(".bear__bandana-tie", {
  transformOrigin: "0 0",
  rotate: -30,
  ...shared
}, 0).to(".bear__keyboard-arm", {
